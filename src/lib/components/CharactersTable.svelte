...
<Table.Root>
	...
	<Table.Body>
		{#each characterService.characters as character (character.id)}
			<Table.Row>
				...
				>
				<Table.Cell class="text-center">{character.cpuWL.wins}/{character.cpuWL.losses}</Table.Cell>
	...
	<Table.Footer>
		<Table.Row>
			<Table.Cell colspan={9}>Total wins</Table.Cell>
			<TableCell>
				<div class="flex flex-row justify-center gap-1">
					<UserIcon size="20" />
					{globalWLService.globalWL.wins}
					<div></div>
					<CpuIcon size="20" />
					{globalWLService.globalWL.losses}
				</div>
			</TableCell>
			<Table.Cell />
		</Table.Row>
